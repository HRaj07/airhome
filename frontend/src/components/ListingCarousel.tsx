...
export default function ListingCarousel({
  ...
  const arrowClass = (enabled: boolean) =>
    `flex h-8 w-8 items-center justify-center rounded-full border border-neutral-200 bg-white transition-opacity dark:border-neutral-700 dark:bg-neutral-900 ${
      enabled ? "hover:shadow-md" : "opacity-30 cursor-default"
    }`;

  return (
    <section className="py-3">
      <div className="mb-3 flex items-center justify-between">
        <Link href={`/?location=${encodeURIComponent(city)}`} className="group flex items-center gap-2">
        ...
        <div className="hidden items-center gap-2 sm:flex">
          <button onClick={() => scrollBy(-1)} disabled={!canPrev} aria-label="Scroll left" className={arrowClass(canPrev)}>
            <ChevronLeft size={16} />
          </button>
          <button onClick={() => scrollBy(1)} disabled={!canNext} aria-label="Scroll right" className={arrowClass(canNext)}>
